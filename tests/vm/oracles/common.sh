...
p814_must_close() {
  ...
  [ "${_got}" -ne 0 ] \
    || die "missing tests/oracles/p8-$1.sh (P8.14 incomplete)"
}

p814_probe_unset() {
  # Never invoke installer/enact/TUI against live /srv/aios. Destroot only.
  _empty="${P814_TMP}/unset-probe"
  mkdir -p "${_empty}/boot"
  _enact="${P814_ROOT}/payload/profile/airootfs/usr/lib/aios/bin/enact"
  AIOS_ROOT="${_empty}" "${_enact}" synthesise work-runtime \
    >/dev/null 2>"${P814_TMP}/unset-enact.err" || true
  grep -Eq 'HI-15|envelope accept|AIOS_ROOT' "${P814_TMP}/unset-enact.err" \
    || die "destroot synthesise without accept must refuse: $(cat "${P814_TMP}/unset-enact.err")"
  printf 'quit\n' \
    | AIOS_ROOT="${_empty}" AIOS_BOOTSTRAP="${_empty}/boot" AIOS_HI="${HI}" \
      python3 -u "${MAIN}" \
      >/dev/null 2>"${P814_TMP}/unset-installer.err" || true
  printf 'quit\n' \
    | AIOS_ROOT="${_empty}" AIOS_ANSWERS="${_empty}/no-answers.json" \
      AIOS_BRAKE="${P814_TMP}/unset-brake" \
      python3 -u "${TUI}" \
      >/dev/null 2>"${P814_TMP}/unset-tui.err" || true
  if [ -e /srv/aios/state/bootstrap-in-progress ] \
    && [ "${P814_BIP_BEFORE}" -eq 0 ]; then
    die "unset probe created live bootstrap-in-progress"
  fi
  p814_assert_isolation
}
